...
from utils.constants import (
    ELEVATOR_LEADING_MOTOR_ID,
    ELEVATOR_FOLLOWING_MOTOR_ID,
    CLIMB_MOTOR_ID,
    ROTATE_INTAKE_MOTOR_ID,
    TOP_WHEELS_MOTOR_ID,
    single_rotation_inches,
)
...
class RobotContainer:
    ...
    def __init__(self) -> None:
        ...
        self.vision = LimelightAlign(self.drivetrain)
    ...
    def _configure_drivetrain(self) -> None:
        self._max_speed = TunerConstants.speed_at_12_volts * 0.75
    ...
    def _configure_drivetrain_controls(self) -> None:
        ...
        self._joystick.rightBumper().whileTrue(self.vision)
    ...
    def _configure_elevator_controls(self) -> None:
        ...
        self._functional_controller.y().whileTrue(
            cmd.runEnd(
                lambda: self.elevator.move_motor(20), lambda: self.elevator.brake_manual()
            )
        )
        ...
        self._functional_controller.pov(0).whileTrue(
            self.elevator.move(ElevatorPositions.Level4, ElevatorMode.POSITION)
        )  # top
        self._functional_controller.pov(90).whileTrue(
            self.elevator.move(ElevatorPositions.Level3, ElevatorMode.POSITION)
        )  # right
        self._functional_controller.pov(180).whileTrue(
            self.elevator.move(ElevatorPositions.Level1, ElevatorMode.POSITION)
        )  # bottom
        self._functional_controller.pov(270).whileTrue(
            self.elevator.move(ElevatorPositions.Level2, ElevatorMode.POSITION)
        )  # left
    ...
    def _configure_climb_controls(self) -> None:
        # Climb controls
        self._functional_controller.rightBumper().whileTrue(self.climb.run(55))
        self._functional_controller.leftBumper().whileTrue(self.climb.run(-55))
        # self._functional_controller.x().whileTrue(self.climb.hold_position())
    ...
    def _load_autonomous_routines(self) -> None:
        ...
        autons_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "autonomous", "autons"
        )
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module):
                        if (
                            ...
                            display_name = (
                                " ".join(
                                    word.capitalize() for word in auto_name.split("_")
                                )
                                + " Auto"
                            )
